...
class Qwen2VLTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        if self.is_loaded:
            print('{} is already loaded, `load_model` called again, skipping.'.format(self.vision_tower_name))
            return
        print(f"Loading vision tower from {self.vision_tower_name}...")
...
class Qwen2AudioTower(nn.Module):
    ...
    def image_processor(self, image, *args, **kwargs):
        if isinstance(image, str):
            ...
            audio, sr = image
            if sr != self._image_processor.feature_extractor.sampling_rate:
                audio = librosa.resample(audio, orig_sr=sr, target_sr=self._image_processor.feature_extractor.sampling_rate)
                sr = self._image_processor.feature_extractor.sampling_rate
